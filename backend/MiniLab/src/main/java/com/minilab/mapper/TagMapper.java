...
public interface TagMapper {
    @Insert("insert into minilab.emp_tag (emp_id) value #{id}")
    ...
    @Insert("insert into minilab.machine_tag (machine_id) value #{id}")
    void initialMachineTag(Integer id);

    @Update("update minilab.machine_tag set tags = #{tags}, update_time = #{updateTime} where emp_id = #{empId}")
    void updateMachineTagById(MachineTag tag);

    @Delete("delete from minilab.machine_tag where emp_id = #{id}")
