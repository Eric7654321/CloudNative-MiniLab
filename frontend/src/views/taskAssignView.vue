...
import {
  NLayout, NLayoutHeader, NLayoutContent, NDataTable, NTag, NTooltip, NText,
  NH2, NIcon, NCard, NButton, NModal, NForm, NFormItem,
  NInput, NDatePicker, NSelect, NSpace, NInputNumber, NPopconfirm, NCheckbox, NCheckboxGroup, // Added NCheckbox, NCheckboxGroup
    ...
    if (Array.isArray(arr) && arr.length > 0) {
      ...
      if (remainingItemsCount > 0) {
        return h(NTooltip, null, {
          ...
          default: () => h('div', { style: 'max-width: 300px; word-break: break-all;' }, arr.join(', ')),
        });
      }
...
const formRules: FormRules = {
  ...
  startTime: [{ required: true, message: '請選擇開始時間', trigger: ['change', 'blur'], type: 'string' }],
  // Validation for selectedMachineIds and selectedSkillTags can be added if needed, e.g., at least one selected
  machineIds: [ // This is a placeholder path, actual validation logic would be on selectedMachineIds
    {
      validator: (rule: FormItemRule, value: any) => { // value here is not directly used. We check selectedMachineIds
...
const handleEdit = (row: Task) => {
  ...
  selectedSkillTags.value = row.tag ? row.tag.split(',').filter(t => t) : [];
  ...
  formRef.value?.validate(async errors => {
    if (!errors) {
      ...
      taskToProcess.machineName = JSON.stringify(
        selectedMachineIds.value.map(id => { // id here is a machine_id
      ...
      taskToProcess.tag = selectedSkillTags.value.join(',');
  ...
  dialog.info({
    ...
    onPositiveClick: async () => {
      ...
      try {
        ...
        // 3. Handle success
        if (response.status === 200 || response.status === 201) { // Or other success codes
          ...
          message.error(`提交失敗：伺服器回應狀態 ${response.status}. ${response.data?.message || ''}`);
    ...
    onPositiveClick: async () => {
      try {
        ...
        if (response.status === 200 || response.status === 204) {
          ...
          message.error(`刪除任務失敗：伺服器回應狀態 ${response.status}. ${response.data?.message || ''}`);
...
const createColumns = ({ onEdit, onDelete }: { onEdit: (rowData: Task) => void; onDelete: (rowData: Task) => void }): DataTableColumns<Task> => [
  { title: 'ID', key: 'id', width: 60, fixed: 'left', sorter: 'default', resizable: true },
  { title: '員工ID', key: 'emp', width: 80, sorter: 'default', resizable: true, render: (row) => row.emp ?? 'N/A' },
  { title: '員工名稱', key: 'empName', width: 100, resizable: true, ellipsis: { tooltip: true } },
  {
    title: '機台ID', key: 'machine', width: 150, ellipsis: { tooltip: { placement: 'top', width: 300, style: { maxWidth: '300px' } } },
    render(row) { return renderJsonArray(row.machine); }, resizable: true
  },
  {
    title: '機台名稱', key: 'machineName', width: 180, ellipsis: { tooltip: { placement: 'top', width: 300, style: { maxWidth: '300px' } } },
    render(row) { return renderJsonArray(row.machineName); }, resizable: true
  },
  {
    title: '開始時間', key: 'startTime', width: 180, render: (row) => formatDateTime(row.startTime),
    sorter: (a, b) => new Date(a.startTime || 0).getTime() - new Date(b.startTime || 0).getTime(), resizable: true
  },
  {
    title: '結束時間', key: 'endTime', width: 180, render: (row) => formatDateTime(row.endTime),
    sorter: (a, b) => new Date(a.endTime || 0).getTime() - new Date(b.endTime || 0).getTime(), resizable: true
  },
  { title: '技能標籤', key: 'tag', width: 100, resizable: true, ellipsis: { tooltip: true } },
  {
    title: '任務描述', key: 'description', width: 200, ellipsis: { tooltip: { placement: 'top', width: 300, style: { maxWidth: '300px' } } },
    resizable: true
  },
  { title: '群組ID', key: 'group', width: 90, resizable: true },
  { title: '更新者ID', key: 'updaterId', width: 90, resizable: true, render: (row) => row.updaterId ?? 'N/A' },
  {
    title: '是否完成', key: 'isFinish', width: 100, align: 'center',
    render(row) {
      return h(NTag, { type: row.isFinish === 1 ? 'success' : 'warning', size: 'small', round: true, },
        { default: () => (row.isFinish === 1 ? '已完成' : '未完成') }
      );
    },
    filterOptions: [{ label: '已完成', value: 1 }, { label: '未完成', value: 0 }],
    filter(value, row) { return row.isFinish === value }, resizable: true
  },
  {
    title: '更新時間', key: 'updateTime', width: 180, render: (row) => formatDateTime(row.updateTime),
    sorter: (a, b) => new Date(a.updateTime || 0).getTime() - new Date(b.updateTime || 0).getTime(), resizable: true
  },
  {
        ...
        h(NPopconfirm,
          { onPositiveClick: () => onDelete(row), positiveText: "確認刪除", negativeText: "取消" },
          {
            trigger: () => h(NTooltip, null, {
              trigger: () => h(NButton, { strong: true, tertiary: true, circle: true, size: 'small', type: 'error' },
...
const bufferColumns = computed((): DataTableColumns<Task> => [
  { title: '員工名稱', key: 'empName', width: 100, resizable: true, ellipsis: { tooltip: true } },
  {
    title: '機台名稱', key: 'machineName', width: 150, // Changed from machine to machineName for buffer display
    render(row) { return renderJsonArray(row.machineName); }, resizable: true
  },
  {
    title: '開始時間', key: 'startTime', width: 180, render: (row) => formatDateTime(row.startTime), resizable: true
  },
  { title: '任務描述', key: 'description', width: 200, ellipsis: { tooltip: true }, resizable: true },
  { title: '技能標籤', key: 'tag', width: 100, resizable: true },
  {
    title: '操作', key: 'actions', width: 80, align: 'center', fixed: 'right',
    render(row) {
      return h(NTooltip, null, {
        trigger: () => h(NButton, { strong: true, tertiary: true, circle: true, size: 'small', type: 'warning', onClick: () => handleRemoveFromBuffer(row) },
          { icon: renderIcon(DeleteIcon) }
        ),
        default: () => '從緩衝區移除'
      });
...
const calculateMaxHeight = () => {
  ...
  const bufferTableApproxHeight = bufferedTasks.value.length > 0 ? (Math.min(bufferedTasks.value.length, 5) * 40 + 80) : 0;
  const extraSpacing = 20;
  tableMaxHeight.value = window.innerHeight - headerHeight - contentPaddingY - paginationHeight - bufferTableApproxHeight - extraSpacing;
...
const loadData = async () => {
  try {
    ...
    const EmpapiData: Employee[] = EmpApiRes?.data.data || []
    employeeOptions.value = EmpapiData.map(item => ({
      label: item.name,
      value: item.id
    }))
